...
export class AuthenticationService implements OnInit, CanActivate, CanActivateChild {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
    // return true;
    const indicator = JSON.parse(localStorage.getItem(`${this.storagePrefix}isAuthenticated`));
  ...
  initSession() {
    // this.http.get(this.userInfoUrl).subscribe((userInfo: { username: string, authorities: AuthorityModel[], role: string }) => {
